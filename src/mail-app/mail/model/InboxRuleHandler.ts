...
import { assertMainOrNode, InboxRuleType, MailSetKind } from "@tutao/app-env"
...
export class InboxRuleHandler {
	...
	async findMatchingInboxRule(
		...
		mailDetails?: tutanotaTypeRefs.MailDetails | null,
	): Promise<Nullable<{ targetFolder: tutanotaTypeRefs.MailSet; processInboxDatum: UnencryptedProcessInboxDatum; excludeFromSpamFilter: boolean }>> {
		...
		const inboxRules = this.logins.getUserController().props.inboxRules
...
export async function _findMatchingRule(
	...
	mailDetails: tutanotaTypeRefs.MailDetails | null = null,
...
async function checkInboxRule(
	mailFacade: MailFacade,
	mail: tutanotaTypeRefs.Mail,
	inboxRule: tutanotaTypeRefs.InboxRule,
	mailDetails: tutanotaTypeRefs.MailDetails | null = null,
): Promise<boolean> {
...
function usesMailDetails(rules: readonly tutanotaTypeRefs.InboxRule[]): boolean {
...
function _matchesRegularExpressionCaseInsensitive(value: string, inboxRule: tutanotaTypeRefs.InboxRule): boolean {
	if (isRegularExpression(inboxRule.value)) {
		let flags = inboxRule.value.replace(/.*\/([gimsuy]*)$/, "$1")
		let pattern = inboxRule.value.replace(new RegExp("^/(.*?)/" + flags + "$"), "$1")
